...
import { isPreviewMode, setPreviewMode, getSavedPreviewIndex } from './usePreviewState'
...
watch(plugin, (newPlugin) => {
  if (newPlugin.type === 'loaded') {
    ...
    // Restore preview index when entering portal from a fresh iframe load
    if (isPreviewMode.value) {
      currentFileIndex.value = getSavedPreviewIndex()
    }
...
const checkExist = (file: any) => {
  return files.value.some((item) => {
    isItemWithUuid(item) && item.uuid === file.uuid
  })
}
...
const refreshAssets = async () => {
  ...
  const promises = files.value.map(async (file: any, index: number) => {
    ...
    try {
      const oldCDN = new URL(file.cdn);
      const oldParams = oldCDN.searchParams;
      ...
      let newParams = new URL(cdn).searchParams;
      if (newParams.has("force_format")) {
        let force_format = newParams.get("force_format");
...
const previewAsset = (file: any, index: number) => {
  currentFile.value = file
  currentFileIndex.value = index
  setPreviewMode(true, index)
  if (plugin?.actions) plugin.actions.setModalOpen(true)
}

const navigateFile = (index: number) => {
  currentFileIndex.value = index
  ...
  <div v-if="plugin.type === 'loaded'">
    ...
    <div class="missConfig" v-if="!isValid">
      ...
      <div>
        ...
        <strong>cname</strong> is optional (Custom CNAME). Check your CNAME configuration in Hub <a href="https://hub.scaleflex.com/settings/dmo/delivery/cname" target="_blank">here</a>
      </div>
      <div>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
             class="size-6">
        ...
        <strong>limit</strong> is optional (limit for all files type ex: 3)
      </div>
      <div>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
             class="size-6">
      ...
      <div>
        ...
        <strong>limitType</strong> is optional (ex: image, document, video, audio)
      </div>
      <div>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
             class="size-6">
    ...
    <div v-if="isValid">
            ...
            <template v-for="(file, index) in files">
              <div class="thumb-wrapper" v-if="file && checkFileIncludesImage(file)">
                <div class="thumbnail" :key="index">
                  ...
                  <div class="preview-icon cursor-pointer" @click="() => previewAsset(file, index)">
              ...
              <div class="thumb-wrapper" v-if="file && !checkFileIncludesImage(file)">
                <div class="thumbnail notimage" :key="index">
                  ...
                  <div v-if="documentArr.includes(getTypeAssets(file.type))" class="preview-icon cursor-pointer"
                       @click="() => previewAsset(file, index)">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                      <path
                        d="M15 12c0 1.654-1.346 3-3 3s-3-1.346-3-3 1.346-3 3-3 3 1.346 3 3zm9-.449s-4.252 8.449-11.985 8.449c-7.18 0-12.015-8.449-12.015-8.449s4.446-7.551 12.015-7.551c7.694 0 11.985 7.551 11.985 7.551zm-7 .449c0-2.757-2.243-5-5-5s-5 2.243-5 5 2.243 5 5 5 5-2.243 5-5z" />
                    </svg>
      ...
      <div class="flex" v-if="!plugin.data.isModalOpen">
        ...
        <div v-if="getIsOverLimit()" class="column exceeds-the-limit">
          <svg xmlns="http://www.w3.org/2000/svg" fill="red" version="1.1" width="12px" height="12px"
